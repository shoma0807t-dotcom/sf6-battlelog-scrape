...
**Playwright（ヘッドレスブラウザ）は使いません。** ログインも対戦履歴の取得も、素のHTTPリクエスト
（`fetch`）だけで行います。ブラウザを一切起動しないので、Bot判定（Cloudflare等）の対象になりようが
ありません。結果を**GitHubのGist（非公開）**にアップロードします。

## 重要：ログイン部分は一部未検証です

CAPCOM IDのログイン（`auth.cid.capcom.com/usernamepassword/login`）へ送るデータの内容は、
実際にDevToolsで確認したものをそのまま使っています。ただし、

- ログインページから `state` / `_csrf` をどう抜き出すか（Auth0 Lockの典型パターンで実装していますが未検証）
- ログインPOST後に何が返ってくるか（302リダイレクトか、自動送信フォームを含むHTMLか）

の2点は実測できていません。失敗した場合は `output/debug-login-*.html` にその時点のレスポンスが
保存されるので、それを見ながら1回だけ調整が必要になる可能性があります。

**それでも動かない・調整が面倒な場合は、Cookie方式にフォールバックできます**（後述）。
